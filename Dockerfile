...
# Install tedana (python 3)
COPY tedana_env.yml /tmp/
COPY tedana_wrapper.sh /
RUN conda env create -f /tmp/tedana_env.yml && \
  rm /tmp/tedana_env.yml && \
  chmod +x /tedana_wrapper.sh
